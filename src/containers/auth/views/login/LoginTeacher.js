...
const LoginTeacherPage = (props) => {
    ...
    const user = useSelector(state => state.user.teacher.result);
    ...
    const onSubmit = (values) => {
        const callback = () => {
            ...
            if (json_token.role === 1 && json_token.role === roleCheck) {
                setTimeout(() => {
                    dispatch(userActions.logoutUser());
                }, 18000000);
                dispatch(userActions.getUserTeacher({ giao_vien_id: json_token.userId, isUpdateStorage: true }));
            } 
            else {
                notification.error({
        ...
        dispatch(userActions.loginUser( { type: 2, login: values } , callback));
    };
                ...
                <Row className="logo" align={'middle'}>
                    <Col xs={{ span: 24 }} lg={{ span: 4 }}>
                        <NavLink to='/luyen-tap/trang-chu' className="logo">
                            <Avatar shape="square" size={200} src={require('assets/rank/Logo-enno.png').default} />
                        </NavLink>
                    </Col>
